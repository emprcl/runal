...
func (c *Canvas) BackgroundText(text string) {
	...
	if len(text) == 0 {
		...
		c.backgroundRunes = []rune(defaultBackgroundText)
		return
	}
	c.backgroundText = text
	c.backgroundRunes = []rune(text)
}
...
func (c *Canvas) FillText(text string) {
	...
	if len(text) == 0 {
		...
		c.fillRunes = []rune(defaultFillText)
		return
	}
	c.fillText = text
	c.fillRunes = []rune(text)
}
...
func (c *Canvas) StrokeText(text string) {
	...
	if len(text) == 0 {
		...
		c.strokeRunes = []rune(defaultStrokeText)
		return
	}
	c.strokeText = text
	c.strokeRunes = []rune(text)
}
